...
в каналах. Все они неагрессивны и съедобны."""
    ...
    Planet(
        "Прерия-2",
        """Планета, колонизированная США. Представляет собой идеализированный
вариант Дикого Запада. Население — более двадцати тысяч колонистов, кроме
того, имеется собственная разумная раса («индейцы»), очень похожая на людей,
но стоящая на стадии первобытного скотоводства."""
    ),
    Planet(
        "Аранк",
...
специальных приспособлений смертельно для человека."""
    ...
    Planet(
        "Шеали",
        """Планета, населённая расой шеали — нелетающими птицами, уникальность
которых состоит в том, что они научились полностью гасить в себе разум по
достижении совершеннолетия. Таким образом, они избавились от душевных терзаний
и тревожных размышлений. В повседневной жизни взрослые шеали не думают, а
действуют благодаря рефлексам, приобретённым в детстве."""
    ),
    Planet(
        "Талисман",
        """Планета, покрытая двухсотметровым слоем тумана, перерабатывающим
излучение местного солнца в электричество, поступающее затем в землю. Эта
энергия расходуется на непрерывный синтез и последующее разрушение материи в
так называемых сейфах."""
    ),
]


class Spectre(SluggedWorld):
    def __init__(self, **data):
        data['loader'] = self.__custom_loader
        super().__init__(**data)

    def __custom_loader(self):
        wiki = self.wiki_loader()
